// formal/f_fabric_conservation_l12.v -- whitebox-strengthened copy of
// f_fabric_conservation.v (identical model + ledger; see the original
// for the full header). The ONLY additions are the L1/L2 strengthening
// lemmas of docs/ACADEMIC-RIGOR.md §3.4, stated here as whitebox
// assertions into u_pipe / u_coreB internals. This Yosys' Verilog
// frontend implements neither hierarchical references nor bind, so the
// harness declares undriven peek wires (f_ws_*) read by the asserts, and
// formal/fabric.conservation.prove-l12.sby connects them to the real
// signals with flatten + rename + connect (the sby script is part of
// the harness; rtl/ is untouched):
//
//   L1 (pipe content)  every flit resident in or emitted by the pipe
//       post-setup has op = OP_EFF, src = A_ID, dst = B_ID;
//   L2 (command provenance)  core B's hb_cmd train strobes occur only
//       as commits of genuinely accepted effect ops -- plus the
//       one-op-FSM commit exclusivity that makes in-service <= 1
//       (SER) inductive: a booking strobe only exists inside the
//       ST_EFFT->ST_EFFR->ST_EFFP->ST_EFFM->ST_EFFI chain (q_cell_core.v
//       ~385-435), and an accepted-but-unbooked effect keeps the core
//       inside that chain.
//
...
module f_fabric_conservation(input clk, input rst_n);
    ...
    // -------- L1/L2 whitebox peek wires (driven by the sby script) ----
    wire        f_ws_av, f_ws_bv;   // u_pipe.a_v / u_pipe.b_v
    wire [74:0] f_ws_aq, f_ws_bq;   // u_pipe.a_q / u_pipe.b_q
    wire [4:0]  f_ws_stB;           // u_coreB.state
    wire [3:0]  f_ws_lrB;           // u_coreB.lr_src

    always @(posedge clk) if (rst_n) begin
        // -------- L1 (pipe content), whitebox on u_pipe ---------------
        // Every flit resident in (a_q main, b_q skid) or emitted by the
        // pipe post-setup has op = OP_EFF, src = A_ID, dst = B_ID.
        // Flit bus layout {op,src,dst,a0,a1,a2,dat}, FW = OPW+2*AIDW+4*PW:
        //   op [74:72], src [71:68], dst [67:64].
        if (f_run) begin
            if (f_ws_av) begin
                assert (f_ws_aq[74:72] == OP_EFF);
                assert (f_ws_aq[71:68] == A_ID);
                assert (f_ws_aq[67:64] == B_ID);
            end
            if (f_ws_bv) begin
                assert (f_ws_bq[74:72] == OP_EFF);
                assert (f_ws_bq[71:68] == A_ID);
                assert (f_ws_bq[67:64] == B_ID);
            end
        end

        // -------- one-op FSM commit exclusivity (SER support) ---------
        // A booking strobe (hb_cmd 001/101 at B) exists only while the
        // core is inside the effect chain that a genuinely accepted
        // effect op entered.
        if (f_bookB_stb) begin
            assert (f_ws_stB == 5'd8  ||   // ST_EFFR
                    f_ws_stB == 5'd20 ||   // ST_EFFP
                    f_ws_stB == 5'd21 ||   // ST_EFFM
                    f_ws_stB == 5'd19);    // ST_EFFI
        end
        // In-service > 0 pins the core inside the effect chain (or on the
        // very accept cycle, still at ST_IDLE): the one-op FSM cannot
        // accept a second effect before booking the first.
        if (f_ins != 0) begin
            assert (f_ws_stB == 5'd6  ||   // ST_EFF
                    f_ws_stB == 5'd7  ||   // ST_EFFT
                    f_ws_stB == 5'd8  ||   // ST_EFFR
                    f_ws_stB == 5'd20 ||   // ST_EFFP
                    f_ws_stB == 5'd21 ||   // ST_EFFM
                    f_ws_stB == 5'd19 ||   // ST_EFFI
                    (f_ws_stB == 5'd2 && f_effB)); // ST_IDLE accept cycle
        end

        // -------- L2 (command provenance) ------------------------------
        // A train strobe at B is a commit of an effect genuinely accepted
        // from A: the chain's lr_src is the linked peer.
        if (f_bookB_stb) assert (f_ws_lrB == A_ID);
        // Accept-side provenance (op/src/dst of what B pops) is exactly
        // L1 on u_pipe.a_q (m_* are a_q), so it needs no separate lemma here.
