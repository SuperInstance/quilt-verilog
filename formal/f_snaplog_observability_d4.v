...
// WINDOW VARIANT (w24): identical obligations, 24-fire journal window
// (still admits log saturation 16 + 8 overflow drops); the 64-deep
// window variant is the deep companion (snaplog.t6.sby).
...
module f_snaplog_observability_d4(input clk, input rst_n);
    localparam PW = 16, AIDW = 4, TICKW = 24, DEPTH = 4, MAG = 1;  // d4 variant
    localparam EW = TICKW + 1 + AIDW + PW;   // 45
    localparam IDXW = 2;                     // clog2(DEPTH)
    localparam JOURNAL = 8, JW = 3;         // full-wave shadow depth (window variant)
    ...
    // journal as a SHIFT REGISTER (newest at 0), mirroring the DUT's own
    // structure: comparator-friendly (no dynamic index arithmetic -- the
    // array+mux encoding stalled two solvers at the saturation frontier).
    // JOURNAL (24) > DEPTH (16): while i < o_count the entry is always
    // inside the journal regardless of total fires.
    reg [EW-1:0] f_journal [0:JOURNAL-1];
    reg [JW+2:0] f_fires = 0;                     // total observable fires
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            // journal shift (the wave records everything observable)
            if (fire_now) begin
                for (j = JOURNAL-1; j > 0; j = j - 1) f_journal[j] <= f_journal[j-1];
                f_journal[0] <= entry;
    ...
    always @(posedge clk) if (rst_n) begin
        ...
        if ({1'b0, f_ridx} < o_count)
            assert (o_rent == f_journal[f_ridx]);
    ...
    always @(posedge clk) if (rst_n) begin
        ...
        cover (o_drops >= 2);                        // overflow drops
