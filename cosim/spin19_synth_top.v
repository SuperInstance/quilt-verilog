// spin19_synth_top.v -- LC/fmax cost probe wrapper: q_wall_gate with
// registered IO so nextpnr times reg->comb->reg paths (SPIN-19 SYNTH).
// Parameters are set by the yosys chparam in the per-arm scripts.
module spin19_synth_top (
    input  wire clk,
    input  wire rst_n,
    input  wire i_go,
    input  wire [31:0] i_seed,
    input  wire [41:0] i_lats,
    output wire [47:0] o_resid,
    output wire o_tval,
    output wire [47:0] o_events
);
    reg         r_go   = 0;
    reg  [31:0] r_seed = 0;
    reg  [41:0] r_lats = 0;
    reg  [47:0] o_resid_r = 0;
    reg         o_tval_r  = 0;
    reg  [47:0] o_events_r = 0;
    ...
    q_wall_gate #(.N(7), .K(1), .PD(3), .GMODE(2), .THETA100(8'd110))
