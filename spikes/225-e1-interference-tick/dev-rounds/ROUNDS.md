...
- **Verdict:** DONE (deliverable filed as ROUND-22-regimemap.md; pre-reg 3e9a3a7, result 3e9ab40) — both pre-registered gates fired against the interesting story. M-K2: K=2 is Δ-flat (walls 4,4 at Δ=8/16) — interference-threshold mechanism FALSIFIED; the flat/stratified line sits between K=1 and K=2, not at K=4. M-PD4: comp regime extends through pd=4 (pd*∈(3,4]) → 2pd*=6 NOT doing double duty; STUDENT structural-link candidate demoted to coincidence (Δ=12-scoped). Surprise: entire Δ=12 grid (K∈{2,4,8}×pd∈{2,3,4,6}) structureless, walls 3–4 — pd-stratification is Δ-carried, not a fixed (K,pd) property. Canary: octave replays 2/3/4 exact PASS.
- **Next rung (named):** Δ-resolved map at K=2 vs K=1 — is K=2 flat at ALL Δ or only ≤16 (crosses to F2 at Δ≥24)? If never crosses, K-line is a true phase boundary and only K=1 needs a pd-stratified model.
- **Headline:** K=2 Δ-flat — regime line is K=1 vs K≥2; pd stratification is Δ-carried

## Round 23 — Δ-resolved K=2 vs K=1 comp-wall map (round 22's named rung) — DONE (pre-reg cc46c9e)
- **Item:** Is K=2 Δ-flat at ALL Δ or does it cross to F2=⌈2·r^0.25⌉ at Δ≥24? If never crosses, the K-line is a true phase boundary and only K=1 needs a pd-stratified model. Grid: K=2 × Δ{8,12,16,20,24,32,48} × pd{2,3}, comp arm, calm, N 2..13; K=1 controls at Δ{8,16,32}×pd{2,3}.
...
- **Commit:** 485560e (pre-reg) + result commit
