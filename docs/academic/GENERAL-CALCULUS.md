...
**Statement registry.** 20 definitions (GC-D1–D20), 5 preliminary definitions (GC-P0.1–P0.8), 12 theorems (GC-T1–T12), 5 lemmas/propositions (GC-L1–L5), 5 counterexamples (GC-X1–X5), 4 conjectures (GC-C1–C4), 34 numbered proofs. Grade: **pen-only** throughout (§8 specifies the benches; each conjecture carries its registered falsifier).
...
>
> *Registered falsifier.* A wavefront simulator with fixed buffer bound B, no drops, no blocking, and a simulation proof covering *all* local-tick quilts (the artifact: the machine, the proof, and the treatment of the burst family — the source that emits k deliveries between barriers for adversarial k). Alternatively, a lower-bound proof (unboundedness for all wavefront simulators) resolves the conjecture positively. The historical stake: PLATO's controller is the bounded-B special case where the *source* is protocol-shaped to fit the buffer (write-only-changes); the conjecture says that shape was necessity, not thrift. *Grade: (a) proved; (b) open.*
...
A falsification of any proved statement is a publishable trace (the house rule); a falsification of a conjecture is the registered artifact doing its job. None of these benches exists today; their absence is this paper's gap entry in the register (G5, DEPENDENCY-GRAPH §6).
