...
> **Theorem 5a (width construction; exact, tight).** With K=8, B=8, PW=16: the readout accumulator (17 bits) cannot overflow — `max Σ C_i·2^(K−i) = (2^B−1)(2^(K+1)−2) = 130,050 < 2^17`; the 16×16 product fits 32 bits exactly; `base + eng` is a 17-bit add whose only exit is the declared saturator. The engine's integer state is therefore *overflow-free by construction*: every saturation is at a named operator, latched sticky, viewable. The headroom is **1022 bits = 0.78%** of the accumulator (130,050 vs 131,072) — **TIGHT** (pinning the 17-bit accumulator, one more bucket bit (B=9) or bucket (K=9) overflows it: 260,610 ≥ 2^17; the widths were swept to silence verilator, and the sweep landed one bit from the edge). Machine-checked: `tb/tb_thm5a_width.v` (B12 closure) + the `q_hebb_edge` elaboration guard, both in `run_suite.sh`.
...
|---|---|---|---|---|
| 1 | Ladder envelope `W/2−1 ≤ Ŵ ≤ 2W+1`, arbitrary arrivals | **Proved** (Thm 1) | None on arrivals; the *law* is exponential (task-level, owned by the acceptance gate) | Existing: `tb/tb_hebb_edge.v` T2 (ONE event schedule, 5 events) + `tb/tb_fabric_smoke.v` exact unshifted golden. **Added 2026-09-03:** `tb/tb_envelope_fuzz.v` (in `run_suite.sh`) — two LCG arrival streams (H=16, H=64; 400 events each; gaps 1..2.5H with 1..5-train bursts), exact continuous golden `2^K·Σ 2^(−(t−te)/H)` accumulated per event, envelope `W/2−1 ≤ Ŵ ≤ 2W+1` asserted at all **800/800** reads plus ovf-clear. **Still open:** the formal k-induction monitor (`tb/formal/`, ABSTRACTION-MATH check #5) — the monitor is the only artifact that covers *all* arrival sequences |
...
| 7 | v1 weight path arithmetic exact (no rounding) | **Proved + machine-checked** (Thm 5a; B12 closed 2026-09-03) | Width construction (K,B,PW as parameterized) | Existing: `tb/tb_hebb_edge.v` T1/T4 (exact readouts), `tb/tb_fabric_smoke.v` (exact wsum). **Added:** `tb/tb_thm5a_width.v` (bound + tightness + identity sweep) and the `q_hebb_edge` elaboration guard (`tb_thm5a_guard` fires it on K=9,B=8); both in `run_suite.sh`. Headroom corrected: 0.78%, not ~3% |
| 8 | Act truncation bias −½ LSB/effect | **Proved** (Thm 5c, floor semantics) — **now MEASURED** (`tb/tb_act_bias.v`, 2026-09-03): err = 63/64/48 vs N/2 = 64/64/48 across three regimes (N=128 dat=+65, N=128 dat=−65, N=96 dat=+321), err/σ ≈ 0.00 — the zero-mean fluctuation model is dead on; floor golden bit-exact at every checkpoint; threshold-shift companion: ideal crosses THRESH at effect 202, RTL at 203 (delay 1 ≤ bound 3, bias delays firing exactly as predicted) | `>>>` = arithmetic floor (as implemented) | Existing: `tb/tb_cell_core.v` saturating golden — *consistent* with the floor by construction. **Closed by `tb/tb_act_bias.v`** (in `run_suite.sh`). Method note: the TB interleaves a view op after every effect — pure gapped effect streams can lose flits (BACKEND-NOTES weak point, under investigation), which would corrupt the measurement; the per-checkpoint floor-model equality assert proves the stream was lossless |
