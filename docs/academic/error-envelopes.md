...
> (1) the age-bucket ladder's factor-2 decay bound; (2) zeroclaw's hyperbolic `[1,4)` interval envelope; (3) RQH's re-injected residue "asymptotically tightening" the envelope; (4) the snap covering inequality `b√n/2 ≤ ε`; (5) the end-to-end fixed-point error of a full hebbian update (`base + ln(1+W)` with ladder `W`). For each: **statement, proof, and a tight/loose grade.** Where the board's prose and the RTL disagree, the RTL and the testbenches win, and the correction is logged. Three corrections were found and are logged in §0 — the reader should see the corrections before the proofs, so the proofs are not read through the wrong lens.

---

## 0. Correction ledger (read first)

Three claims on the board are wrong as written; two of them are implemented correctly in RTL/testbench and only the prose is wrong, one is wrong in the design itself. All three are repaired below.
...
**Proof.** `m = ⌊log₂ W⌋ ⟹ W ∈ [2^m, 2^{m+1}) ⟹ W² ∈ [2^{2m}, 2^{2m+2})`. Hence `2^{2m} ∈ (W²/4, W²]` and `Δ(W) = P₀ >> 2m = ⌊P₀/2^{2m}⌋` satisfies `P₀/W² ≤ Δ(W) < 4P₀/W²` (the floor-1 rule only tightens the lower edge at W=1). ∎
...
> ```
>
> The upper bound is exact and tight. The lower bound requires the additive slack of 1, which is genuinely necessary (the discrete engine reaches W = 0 in finite time; the exact hyperbola never does) and is the slack the testbench asserts.
...
> ```
>
> The as-built deposit `2^g` equals this only when `g = (K + QDW + log₂(1 − 1/(2 ln 2)))/2` — for K = QDW = 8, **g ≈ 7.1, the top of the ladder**. For fresh classes the as-built deposit is too small by a factor `2^(K+QDW−2g)·0.2787` (class 0: ~9,100× too small), and its class-dependence is inverted (largest where the overstatement is smallest). **The as-built RQH does not satisfy its own convergence condition.**
...
**Bottom line (T3):** bounded perturbation ≤ 255 LSB — TIGHT, provable; envelope *preservation* (not tightening) — provable; strong tightening — FALSE as built; exact convergence condition derived and unmet by a factor ~2^(K+QDW−2g)·0.28 (class 0: ~9,100×).
...
| 2 | Hyperbola interval band `[1,4)` | **Proved** (Thm 2a), TIGHT | msb = ⌊log₂W⌋ — exact | Implicit in `tb/tb_hebb_edge.v` T5; **missing**: an explicit interval-band check (decrement cadence vs `P₀/W²`, `4P₀/W²`) and formal check #6 |
| 3 | Hyperbola trajectory `W_true(P₀)−1 ≤ W_rtl ≤ W_true(4P₀)` | **Proved** (Thm 2b): upper exact; lower up to the +1 slack | The +1 slack suffices (proved up to the per-level deficit argument; empirically tight) | Existing: `tb/tb_hebb_edge.v` T5 — but only 5 checkpoints, t ≤ 4096, W₀ = 100. **Missing:** the tail (t up to ~2·τ_{W₀}) where the bound is tightest and the lower slack is exercised, and small cases W₀ ∈ {1,2,3}, P₀ ∈ {2, 4, 8} |
| 4 | RQH boundedness ≤ 255 LSB, no wrap | **Proved** (Thm 3a) | Saturating add, widths | **Missing entirely:** `tb_hebb_rqh.v` "Saturation safety" + "Deposit math" (proposal §5) — the module is sketched, uncompiled, uncommitted |
| 5 | RQH envelope *tightening* | **FALSE as built** (Thm 3b/3c) — convergence condition unmet | — | The proposal's own "Envelope tightening (real golden)" check would *falsify* it; the honest regression is: assert `Ŵ_RQH ∈ [W/2−1, 2W+1+C]` (preservation) and measure, not assert, the centering |
| 6 | Covering `b ≤ 2ε/√n` | **Proved** (Thm 4a), TIGHT | Euclidean metric; judge's metric is a D2 dial | **No tb/ exists — it is a design-time condition on L0 dials.** The validating harness is a judge-consistency test: quantized vs real verdicts over the reachable set including the deep hole, both substrates, mirroring the fleet's already-measured reflex-arc 500-vector integer agreement (that was done in quilt-esp32, not here) |
...
- **T3 (RQH):** bounded perturbation ≤ 255 LSB, no wrap — **TIGHT, proved**; envelope *preserved*, not tightened — strong claim **FALSE as built**; exact convergence condition is `deposit(g) = 2^(K+QDW−g)·0.2787`, unmet by ~9,100× at class 0.
