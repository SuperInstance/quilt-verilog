...
4. **Widths:** the v1 weight path is overflow-free by construction — max readout sum 130,050 < 2^17 with 0.78% headroom (1022 bits, measured), TIGHT (Theorem 5a, error-envelopes.md §5.1); every saturation is at a named operator, sticky, viewable.
...
| B2 | ladder/hyperbola envelope monitors: Theorems 1–2 checked on ONE schedule + census sweeps, not all streams — **HALF-CLOSED 2026-09-03**: random-stream fuzz `tb/tb_envelope_fuzz.v` (in `run_suite.sh`) holds the Thm 1 band at 800/800 reads across two LCG arrival streams (H=16/64, bursts, exact continuous golden); Thm 2 rows already covered by `tb_hyperbola_tail` | §3 | remaining: k-induction formal monitors (ABSTRACTION-MATH §4.3 checks #5/#6) — the only artifacts covering ALL arrival sequences | hours |
...
| B8 | ~~act truncation-bias test missing~~ **CLOSED 2026-09-03**: `tb/tb_act_bias.v` (in `run_suite.sh`) measures the bias against the convergent-rounded ideal — err = 63/64/48 LSB vs N/2 = 64/64/48 across three regimes (positive/negative dat, two strides), err/σ ≈ 0.00; floor-model equality bit-exact at every checkpoint; threshold-shift companion: fire-reachability delayed 1 effect (ideal 202 → RTL 203, bound 3). Theorem 5c is now measured, not just proved | §9 | — (closed) | — |
...
| B12 | ~~width-construction assertion (130,050 < 2^17) is prose~~ **CLOSED 2026-09-03**: `tb/tb_thm5a_width.v` machine-checks the bound, the tightness (B=9/K=9 bust a pinned 17-bit acc), and the always-fits identity (swept K,B 1..12); `q_hebb_edge` carries an elaboration guard enforcing the 17-bit contract (`tb_thm5a_guard` expects it to fire on K=9,B=8). Both wired into `run_suite.sh`. Measured headroom: 1022 bits = 0.78% (the old '~3%' prose was wrong — tighter). | §9 | — (closed) | — |

**Reading of the register:** 12 of 12 claim sections are machine-checked today (§1–§5, §9, §11, §12 end-to-end; §6, §7, §8 as bounded exact-arithmetic enumerators — every instance in the printed bounds, the general theorems still pen; §10 small-scale measured-serialization cosim, commit 3157b3d). Gaps B3/B4/B5 closed-bounded with named residues (composition lemma, δ_min measurement, ε-frontier); B7 half-closed; B8 closed (act bias measured, 2026-09-03); B12 closed (Thm 5a machine-checked + elaboration guard, 2026-09-03). Nothing in the dossier rests on an unrun machine check being secretly run — the register is the boundary, and each line names the artifact that erases it.
...
- **Open gaps: 6 + residues** (B1, B2, B6, B9, B10 open; B7 half-closed; B3/B4/B5 closed-bounded with residues; B11, B12 closed via `7375afb` + the PIPE_EFF=1 re-proof), each with its exact closing artifact and a size estimate.
