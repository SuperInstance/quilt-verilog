...
**Statement registry.** 7 definitions (FC-D1–D7), 4 theorems (FC-T1–T4), 2 counterexamples (FC-X1, FC-X2), 3 propositions (FC-P1–P3), 3 lemmas (FC-L1–L3), 14 numbered proofs. Grade: **pen-only** (§11 names the ~50-line bench that would execute the counterexample and the folds — THE-BREAKDOWN gap B5).
...
Pen-only, as graded in THE-BREAKDOWN (gap B5). The natural bench is ~50 lines of Python on the tapfabric ledger substrate (which already applies transactions nonce-idempotently and additively — FC-L1's machinery, 34/34 tests):
1. replay FC-X1's P₁/P₂ through the balance fold — assert identical compacted forms, differing Q (the counterexample as an executable);
2. a seeded ROM stand-in for h — assert the hiding property (answerer advantage 0 over b) on the two-candidate view;
3. the Λ-fold + Merkle witness protocol of FC-T3 — assert count exactness, inclusion verification, and a rejected forged proof;
4. the Ω(c) bound is a counting argument — a script asserting the 2^c-pairwise-distinct answers on the enumeration family, c swept.
