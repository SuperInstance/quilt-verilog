...
**Statement registry.** 8 definitions (DA-D1–D8), 3 lemmas (DA-L1–L3), 7 theorems (DA-T1–T7), 3 corollaries (DA-C1–C3), 2 propositions (DA-P1–P2), 1 explicit open problem (OP-1), 15 numbered proofs. Grade: **pen-only** (the bench is §7's, shared with `RHO-F-FLOOR.md` §8 — gap B4).
...
Pen-only, with the bench specified and shared with the companion: `floor_bench.py` (THE-BREAKDOWN gap B4; `RHO-F-FLOOR.md` §8) exercises DA-T5's bound and DA-T6(i)'s linear law as *assertions 3* (cost ∝ ρ at fixed ε₀, measured) — and DA-T4's equivalence is directly benchable (drifted-judge verdicts vs prefiltered-judge verdicts, cell by cell, on synthetic frames: any mismatch falsifies the equivalence as stated). The DA-T2 annulus controllability admits a trivial property test (place presented points in the annulus, check both verdicts realizable) — a ~30-line harness on the same fixture. Nothing here is machine-checked today; the grades are in THE-BREAKDOWN's register (§6–§8 family, gap B4).
