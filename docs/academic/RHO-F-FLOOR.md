...
*(Error.)* Under θ¹ at t\*: d_{t\*}(x, a_j) = d₀(x, a_j) + ρF for every non-answer x and every j. Every input x with V_{θ₀}(x) = {k_j} and d₀(x, a_j) ∈ (r − ρF, r] flips to REJECT: verdict error. So err ≥ φ⁺(0, ρF). For the two-sided swept form φ(0, ρF), run the same construction with per-key direction choices (inward for the keys whose inward band is heavier): each key's perturbation direction is independent of the others', and the budget is per-key (RF-D2), so the adversary picks the maximizing direction vector. ∎
